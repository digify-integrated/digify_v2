...
<body id="kt_app_body" data-kt-app-header-fixed-mobile="true" data-kt-app-toolbar-enabled="true"  class="app-default" data-kt-app-page-loading-enabled="true" data-kt-app-page-loading="on">
                    ...
                    <div class="app-main flex-column flex-row-fluid" id="kt_app_main">
                        <div class="d-flex flex-column flex-column-fluid">
                            <div id="kt_app_content" class="app-content flex-column-fluid">
                                <?php 
                                    if($newRecord){
                                        require_once('./apps/settings/language-proficiency/view/_language_proficiency_new.php');
                                    }
                                    else if(!empty($detailID)){
                                        require_once('./apps/settings/language-proficiency/view/_language_proficiency_details.php');
                                    }
                                    else if(isset($_GET['import']) && !empty($_GET['import'])){
                                        require_once('./components/view/_import.php');
                                    }
                                    else{
                                        require_once('./apps/settings/language-proficiency/view/_language_proficiency.php');
                                    }
                                ?>
                            </div>
                        </div>
    ...
    <?php
        $version = rand();
        $scriptFile = './apps/settings/language-proficiency/js/language-proficiency.js';

        if ($newRecord) {
            $scriptFile = './apps/settings/language-proficiency/js/language-proficiency-new.js';
        }
        elseif (!empty($detailID)) {
            $scriptFile = './apps/settings/language-proficiency/js/language-proficiency-details.js';
        }
