...
if(isset($_POST['type']) && !empty($_POST['type'])){
    ...
    switch ($type) {
        # -------------------------------------------------------------
        case 'credential type table':
            $sql = $databaseModel->getConnection()->prepare('CALL generateCredentialTypeTable()');
            $sql->execute();
            ...
            foreach ($options as $row) {
                ...
                $credentialTypeIDEncrypted = $securityModel->encryptData($credentialTypeID);
        ...
        # -------------------------------------------------------------
        case 'credential type options':
            $multiple = (isset($_POST['multiple'])) ? filter_input(INPUT_POST, 'multiple', FILTER_VALIDATE_INT) : false;

            $sql = $databaseModel->getConnection()->prepare('CALL generateCredentialTypeOptions()');
            $sql->execute();
            $options = $sql->fetchAll(PDO::FETCH_ASSOC);
            $sql->closeCursor();
