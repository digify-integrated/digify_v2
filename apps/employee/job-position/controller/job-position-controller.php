...
$controller = new JobPositionController(new JobPositionModel(new DatabaseModel), new AuthenticationModel(new DatabaseModel, new SecurityModel), new SecurityModel(), new SystemModel());
...
class JobPositionController {
    ...
    public function addJobPosition() {
        ...
        $userID = $_SESSION['user_account_id'];
        $jobPositionName = filter_input(INPUT_POST, 'job_position_name', FILTER_SANITIZE_STRING);
        
        $jobPositionID = $this->jobPositionModel->saveJobPosition(null, $jobPositionName, $userID);
    
        $response = [
            'success' => true,
            'jobPositionID' => $this->securityModel->encryptData($jobPositionID),
            'title' => 'Save Job Position',
    ...
    public function updateJobPosition() {
        ...
        $this->jobPositionModel->saveJobPosition($jobPositionID, $jobPositionName, $userID);
    ...
    public function deleteMultipleJobPosition() {
        ...
        if (isset($_POST['job_position_id']) && !empty($_POST['job_position_id'])) {
            ...
            $response = [
                ...
                'message' => 'The selected job positions have been deleted successfully.',
                'messageType' => 'success'
            ];
    ...
    public function exportData() {
        ...
        if (isset($_POST['export_to']) && !empty($_POST['export_to']) && isset($_POST['export_id']) && !empty($_POST['export_id']) && isset($_POST['table_column']) && !empty($_POST['table_column'])) {
            ...
            if ($exportTo == 'csv') {
                $filename = "job_position_export_" . date('Y-m-d_H-i-s') . ".csv";
                ...
                $jobPositionDetails = $this->jobPositionModel->exportJobPosition($columns, $ids);

                foreach ($jobPositionDetails as $jobPositionDetail) {
                    fputcsv($output, $jobPositionDetail);
                }
                ...
                ob_start();
                $filename = "job_position_export_" . date('Y-m-d_H-i-s') . ".xlsx";
                ...
                foreach ($jobPositionDetails as $jobPositionDetail) {
                    $colIndex = 'A';
                    foreach ($tableColumns as $column) {
                        $sheet->setCellValue($colIndex . $rowNumber, $jobPositionDetail[$column]);
                        $colIndex++;
                    }
                    $rowNumber++;
                }
    ...
    public function getJobPositionDetails() {
        ...
        if($total === 0){
            $response = [
                'success' => false,
                'notExist' => true,
                'title' => 'Get Job Position Details',
                'message' => 'The job position does not exist.',
                'messageType' => 'error'
            ];
